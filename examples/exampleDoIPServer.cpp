 ...
 */
void ReceiveFromLibrary(unsigned short address, unsigned char* data,
                        int length) {
 ...
 * @return                  If a positive or negative ACK should be send to the
 * client
 */
...
void listenTcp() {
  ...
  while (true) {
    ...
    connection->setCallback(ReceiveFromLibrary, DiagnosticMessageReceived,
                            CloseConnection);
