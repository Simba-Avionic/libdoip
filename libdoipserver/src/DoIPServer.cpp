...
void DoIPServer::setupTcpSocket() {
  ...
  bind(server_socket_tcp, (struct sockaddr*)&serverAddress,
...
std::unique_ptr<DoIPConnection> DoIPServer::waitForTcpConnection() {
  ...
  return std::unique_ptr<DoIPConnection>(
      new DoIPConnection(tcpSocket, LogicalGatewayAddress));
}
...
int DoIPServer::receiveUdpMessage() {
  ...
  int readBytes = recvfrom(server_socket_udp, data, _MaxDataSize, 0,
...
int DoIPServer::reactToReceivedUdpMessage(int readedBytes) {
  ...
  switch (action.type) {
    case PayloadType::
        VEHICLEIDENTRESPONSE: {  // server should not send a negative ACK if he
                                 // receives the sended
                                 // VehicleIdentificationAnnouncement
      return -1;
    }
    ...
    case PayloadType::VEHICLEIDENTREQUEST: {
      unsigned char* message = createVehicleIdentificationResponse(
          VIN, LogicalGatewayAddress, EID, GID, FurtherActionReq);
      sendedBytes =
...
int DoIPServer::sendUdpMessage(
    unsigned char* message,
    int messageLength) {  // sendUdpMessage after receiving a message from the
                          // client
  // if the server receives a message from a client, than the response should be
  // send back to the client address and port
  clientAddress.sin_port = serverAddress.sin_port;
  clientAddress.sin_addr.s_addr = serverAddress.sin_addr.s_addr;

  int result = sendto(server_socket_udp, message, messageLength, 0,
...
void DoIPServer::setVIN(std::string VINString) { VIN = VINString; }
...
void DoIPServer::setA_DoIP_Announce_Num(int Num) { A_DoIP_Announce_Num = Num; }
...
void DoIPServer::setMulticastGroup(const char* address) {
  ...
  int setPort = setsockopt(server_socket_udp, SOL_SOCKET, SO_REUSEADDR, &loop,
                           sizeof(loop));
  ...
  int setGroup = setsockopt(server_socket_udp, IPPROTO_IP, IP_ADD_MEMBERSHIP,
...
int DoIPServer::sendVehicleAnnouncement() {
  const char* address = "192.168.10.255";

  // setting the destination port for the Announcement to 13401
  clientAddress.sin_port = htons(13400);
  ...
  for (int i = 0; i < A_DoIP_Announce_Num; i++) {
    sendedmessage = sendto(
        server_socket_udp, message, _GenericHeaderLength + _VIResponseLength, 0,
        (struct sockaddr*)&clientAddress, sizeof(clientAddress));
