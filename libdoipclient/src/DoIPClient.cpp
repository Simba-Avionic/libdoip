 ...
 */
void DoIPClient::closeTcpConnection() { close(_sockFd); }

void DoIPClient::closeUdpConnection() { close(_sockFd_udp); }
 ...
 */
const std::pair<int, unsigned char*>*
DoIPClient::buildRoutingActivationRequest() {
  std::pair<int, unsigned char*>* rareqWithLength =
      new std::pair<int, unsigned char*>();
  int rareqLength = 15;
...
void DoIPClient::sendRoutingActivationRequest() {
  const std::pair<int, unsigned char*>* rareqWithLength =
      buildRoutingActivationRequest();
  write(_sockFd, rareqWithLength->second, rareqWithLength->first);
}

/**
 * Sends a diagnostic message to the server
 * @param targetAddress     the address of the ecu which should receive the
 * message
 * @param userData          data that will be given to the ecu
 * @param userDataLength    length of userData
 */
void DoIPClient::sendDiagnosticMessage(unsigned char* targetAddress,
                                       unsigned char* userData,
                                       int userDataLength) {
  unsigned short sourceAddress = 0x0E00;
  unsigned char* message = createDiagnosticMessage(sourceAddress, targetAddress,
                                                   userData, userDataLength);

  write(
      _sockFd, message,
      _GenericHeaderLength + _DiagnosticMessageMinimumLength + userDataLength);
}

/**
 * Sends a alive check response containing the clients source address to the
 * server
 */
void DoIPClient::sendAliveCheckResponse() {
  int responseLength = 2;
  unsigned char* message =
      createGenericHeader(PayloadType::ALIVECHECKRESPONSE, responseLength);
...
void DoIPClient::receiveMessage() {
  ...
  if (!readedBytes)  // if server is disconnected from client; client gets empty
                     // messages
  {
    emptyMessageCounter++;
...
void DoIPClient::receiveUdpMessage() {
  ...
  int readedBytes;
  readedBytes = recvfrom(_sockFd_udp, _receivedData, _maxDataSize, 0,
                         (struct sockaddr*)&_clientAddr, &length);

  if (PayloadType::VEHICLEIDENTRESPONSE ==
      parseGenericHeader(_receivedData, readedBytes).type) {
    parseVIResponseInformation(_receivedData);
  }
}

const std::pair<int, unsigned char*>*
DoIPClient::buildVehicleIdentificationRequest() {
  std::pair<int, unsigned char*>* rareqWithLength =
      new std::pair<int, unsigned char*>();
  int rareqLength = 8;
  unsigned char* rareq = new unsigned char[rareqLength];
...
void DoIPClient::sendVehicleIdentificationRequest(const char* address) {
  ...
  const std::pair<int, unsigned char*>* rareqWithLength =
      buildVehicleIdentificationRequest();

  int sendError =
      sendto(_sockFd_udp, rareqWithLength->second, rareqWithLength->first, 0,
 ...
 * Getter for _sockFD
 */
int DoIPClient::getSockFd() { return _sockFd; }

/*
 * Getter for _connected
 */
int DoIPClient::getConnected() { return _connected; }
